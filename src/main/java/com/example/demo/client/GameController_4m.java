...
public class GameController_4m implements Initializable {
    ...
    private void adjustSpacingVBox(VBox box, int count) {
        ...
        double containerHeight = box.getHeight();

        if (containerHeight <= 0) {
            // Try to get preferred height from the VBox itself
            containerHeight = box.getPrefHeight();

            if (containerHeight <= 0) {
                // Calculate from scene dimensions
                if (box.getScene() != null) {
                    double sceneHeight = box.getScene().getHeight();
                    // Use about 60% of scene height for card containers
                    containerHeight = sceneHeight * 0.6;
                    containerHeight = Math.max(containerHeight, 350);
                } else {
                    containerHeight = 400; // Fallback
                }
            }
        }

        // Account for padding (top=10, bottom=10 from your FXML)
        containerHeight -= 20;

        double spacing;
        if (count * CARD_HEIGHT <= containerHeight) {
            // Cards fit with normal spacing
            spacing = 5;
        } else {
            // Need to overlap cards
            // Total space needed = first_card + (remaining_cards * visible_portion)
            // visible_portion = CARD_HEIGHT + spacing
            // So: CARD_HEIGHT + (count-1) * (CARD_HEIGHT + spacing) = containerHeight
            // Solving for spacing: spacing = (containerHeight - count * CARD_HEIGHT) / (count - 1)
            spacing = (containerHeight - count * CARD_HEIGHT) / (count - 1);

            // Don't let cards overlap more than 75%
            double minSpacing = -CARD_HEIGHT * 0.3;
            spacing = Math.max(spacing, minSpacing);
        }

        System.out.println("VBox " + (box == leftHand ? "LEFT" : box == rightHand ? "RIGHT" : "OTHER") +
                " - Count: " + count + ", Height: " + containerHeight +
    ...
    private void applySpacing() {
        Platform.runLater(() -> {
            ...
            // Use sizes array instead of card lists for opponents
